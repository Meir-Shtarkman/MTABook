...
export async function createPost(text, userId) {
    ...
    posts.push({ text, postId, userName, userId, Time: getDate() })
    await saveData();
    return postId
}
...
export async function sendMessageToUser(text, userTargetId, userId) {
    ...
    if (targetUser) {
        messages.push({ from: { name: sendingUser.name, id: sendingUser.id }, to: { name: targetUser.name, id: targetUser.id }, text, id: generateId(), date: getDate() })
        await saveData();
    } else {
